...
const SocialBubble = ({ 
  ...
  return (
    <motion.a
      ...
      className={`absolute z-30 pointer-events-auto p-4 sm:p-5 rounded-full bg-white/5 backdrop-blur-xl border border-white/20 shadow-[0_8px_32px_rgba(0,0,0,0.3)] text-white hover:text-[var(--color-accent-matcha)] hover:border-[var(--color-accent-matcha)] hover:scale-110 transition-all duration-300 ${className}`}
    >
      {icon === "instagram" && (
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6 sm:w-7 sm:h-7" viewBox="0 0 24 24">
      ...
      {icon === "linkedin" && (
        <svg fill="currentColor" className="w-6 h-6 sm:w-7 sm:h-7" viewBox="0 0 24 24">
  ...
  return (
    <section className="h-screen flex flex-col items-center justify-center relative overflow-hidden bg-black snap-start">
      ...
      >
        ...
        {/* Floating Social Handles (appear on face hover) */}
        <SocialBubble 
